...
(function () {
  ...
  (function initZoomPan() {
    ...
    // -- Apply zoom: physically resize a4Frame inside the fixed viewport,
    //    then re-centre the scroll position so the visible centre stays stable.
    function applyZoom(newZoom, anchorFracX, anchorFracY) {
      // anchorFrac: which point (0–1) within the viewport to zoom toward.
      // Default to centre (0.5, 0.5).
      ...
      // Current scroll offset of the point we're anchoring on
      const oldContentX = viewport.scrollLeft + anchorFracX * vpW;
      const oldContentY = viewport.scrollTop  + anchorFracY * vpH;

      // Scale factor relative to previous zoom
      const ratio = newZoom / zoomLevel;
      zoomLevel = newZoom;

      // Resize the frame — a4Frame fills the viewport at 1×, so at zoom Z
      // its natural size is vpW*Z × vpH*Z.
      a4Frame.style.width  = (vpW * zoomLevel) + 'px';
      a4Frame.style.height = (vpH * zoomLevel) + 'px';

      // Update --a4-w/h so internal font/spacing scales still work correctly.
      // These should stay at the *base* (unzoomed) size so text em-sizes
      // are correct — the frame is now bigger, so content naturally fits.
      // We keep --a4-w at the base viewport width (fonts scale with frame).
      a4Frame.style.setProperty('--a4-w', (vpW * zoomLevel) + 'px');
      a4Frame.style.setProperty('--a4-h', (vpH * zoomLevel) + 'px');

      // Scroll so the anchor point stays under the same screen position.
      const newContentX = oldContentX * ratio;
      const newContentY = oldContentY * ratio;
      viewport.scrollLeft = newContentX - anchorFracX * vpW;
      viewport.scrollTop  = newContentY - anchorFracY * vpH;
    ...
    function resetZoomPan() {
      ...
      const vpW = viewport.offsetWidth;
      const vpH = viewport.offsetHeight;
      a4Frame.style.setProperty('--a4-w', vpW + 'px');
      a4Frame.style.setProperty('--a4-h', vpH + 'px');
