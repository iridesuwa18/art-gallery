...
(function () {
  ...
  function buildLargeLayout(el, art) {
    ...
    `;
    const conceptEl = buildConceptTags(art.concept);
  ...
  function buildMultiLayout(el, artworks, size) {
    ...
    artworks.forEach(art => {
      ...
      // Title + date always shown below the image
      if (art.title || art.date) {
        ...
        imgCaption.innerHTML = `
          ...
          ${art.date  ? `<div class="art-meta">${esc(formatDate(art.date))}</div>` : ''}
        `;
        // Concept tags below title/date in img-col
        const conceptEl = buildConceptTags(art.concept);
        if (conceptEl) imgCaption.appendChild(conceptEl);
        imgCol.appendChild(imgCaption);
      } else {
        // No title/date — still show concept tags if present
        const conceptEl = buildConceptTags(art.concept);
        if (conceptEl) {
          const imgCaption = document.createElement('div');
          imgCaption.className = 'img-col-caption';
          imgCaption.appendChild(conceptEl);
          imgCol.appendChild(imgCaption);
        }
      }

      // Text col: description only (hidden if empty)
      const textCol = document.createElement('div');
      textCol.className = 'text-col';
      if (art.description) {
        textCol.innerHTML = `<div class="art-desc">${esc(art.description)}</div>`;
      } else {
  ...
  // ─── CONCEPT TAGS ──────────────────────────────────────────
  // Renders stored concept fields as minimal labelled tags
  function buildConceptTags(concept) {
    ...
    ORDER.forEach(key => {
      ...
      vals.forEach(v => { if (v) rows.push({ label: LABELS[key] || key, value: v }); });
    });

    if (rows.length === 0) return null;

    const wrap = document.createElement('div');
    wrap.className = 'art-concept-tags';

    rows.forEach(({ label, value }) => {
      const tag = document.createElement('div');
      tag.className = 'art-concept-tag';
      tag.innerHTML = `<span class="act-label">${esc(label)}</span><span class="act-value">${esc(value)}</span>`;
