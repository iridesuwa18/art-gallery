...
(function () {
  ...
  function buildFullscreenOverlay() {
    ...
    // ── Zoom + Pan state ─────────────────────────────────────────
    let fsZoom = 1.0;
    let fsPanX = 0, fsPanY = 0;          // current pan offset in px (pre-scale)
    ...
    function clampPan(wrap) {
      // Max pan is half the extra space created by scaling
      if (!wrap) return;
      const rect = wrap.getBoundingClientRect();
      const maxX = Math.max(0, (rect.width  * fsZoom - rect.width)  / 2 / fsZoom);
      const maxY = Math.max(0, (rect.height * fsZoom - rect.height) / 2 / fsZoom);
    ...
    function commitTransform() {
      ...
      if (fsZoom <= 1) {
        ...
        clampPan(wrap);
        wrap.style.transform = `scale(${fsZoom}) translate(${fsPanX}px, ${fsPanY}px)`;
        wrap.style.transformOrigin = 'center center';
        wrap.style.cursor = 'grab';
      }
    }

    function applyFsZoom(z, pivotX, pivotY) {
      // pivotX/Y: screen coords of zoom pivot (e.g. pinch midpoint). Optional.
      const prevZoom = fsZoom;
      fsZoom = Math.max(FS_MIN, Math.min(FS_MAX, z));

      // Adjust pan so zoom pivots around the given point
      if (pivotX != null && prevZoom !== fsZoom) {
        const wrap = fsContent.querySelector('.fs-page-wrap');
        if (wrap) {
          const rect = wrap.getBoundingClientRect();
          // offset of pivot from wrap centre, in pre-scale space
          const cx = (pivotX - (rect.left + rect.width  / 2)) / prevZoom;
          const cy = (pivotY - (rect.top  + rect.height / 2)) / prevZoom;
          fsPanX += cx * (1 - fsZoom / prevZoom);
          fsPanY += cy * (1 - fsZoom / prevZoom);
        }
      }
    ...
    fsZoomBar.querySelector('[data-dir="1"]').addEventListener('click',  () => applyFsZoom(fsZoom + FS_STEP));
    fsZoomBar.querySelector('.fs-zoom-reset').addEventListener('click',  () => { fsPanX = 0; fsPanY = 0; applyFsZoom(1.0); });

    // Reset zoom + pan when page changes
    fsOverlay._resetZoom = () => { fsPanX = 0; fsPanY = 0; applyFsZoom(1.0); };

    // ── Mouse drag-to-pan ────────────────────────────────────────
    let fsDragging = false, fsDragStartX = 0, fsDragStartY = 0, fsPanStartX = 0, fsPanStartY = 0;

    fsContent.addEventListener('mousedown', e => {
      if (fsZoom <= 1 || e.button !== 0) return;
    ...
    window.addEventListener('mousemove', e => {
      ...
      // Divide by fsZoom because pan is in pre-scale space
      fsPanX = fsPanStartX + (e.clientX - fsDragStartX) / fsZoom;
      fsPanY = fsPanStartY + (e.clientY - fsDragStartY) / fsZoom;
      commitTransform();
    });
    ...
    window.addEventListener('mouseup', () => {
      ...
      fsDragging = false;
      const wrap = fsContent.querySelector('.fs-page-wrap');
      if (wrap && fsZoom > 1) wrap.style.cursor = 'grab';
    });
    ...
    fsContent.addEventListener('wheel', e => {
      ...
      applyFsZoom(fsZoom + delta * 1.2, e.clientX, e.clientY);
      showFsZoomBar();
      scheduleFsZoomHide(1800);
    }, { passive: false });
    ...
    let fsLastPinchMidX = null, fsLastPinchMidY = null;
    let fsTouchPanStartX = 0, fsTouchPanStartY = 0;
    let fsTouchPanPrevX = 0, fsTouchPanPrevY = 0;
    let fsTouchPanning = false;

    fsOverlay.addEventListener('touchstart', e => {
      if (e.touches.length === 2) {
        fsTouchPanning = false;
        const dx = e.touches[0].clientX - e.touches[1].clientX;
        const dy = e.touches[0].clientY - e.touches[1].clientY;
        fsLastPinchDist = Math.hypot(dx, dy);
        fsLastPinchMidX = (e.touches[0].clientX + e.touches[1].clientX) / 2;
        fsLastPinchMidY = (e.touches[0].clientY + e.touches[1].clientY) / 2;
      } else if (e.touches.length === 1 && fsZoom > 1) {
        // Single-finger pan when zoomed
        if (e.target.closest('button,input')) return;
        fsTouchPanning = true;
        fsTouchPanPrevX = e.touches[0].clientX;
        fsTouchPanPrevY = e.touches[0].clientY;
      }
    }, { passive: true });

    fsOverlay.addEventListener('touchmove', e => {
      if (e.touches.length === 2 && fsLastPinchDist !== null) {
        // Pinch zoom with pivot at pinch midpoint
        const dx = e.touches[0].clientX - e.touches[1].clientX;
        const dy = e.touches[0].clientY - e.touches[1].clientY;
        ...
        applyFsZoom(fsZoom + delta, midX, midY);
        // Also pan by midpoint movement
        fsPanX += (midX - fsLastPinchMidX) / fsZoom;
        fsPanY += (midY - fsLastPinchMidY) / fsZoom;
        commitTransform();
        fsLastPinchDist = dist;
        fsLastPinchMidX = midX;
        fsLastPinchMidY = midY;
        showFsZoomBar();
        scheduleFsZoomHide(2000);
      } else if (e.touches.length === 1 && fsTouchPanning && fsZoom > 1) {
        const dx = (e.touches[0].clientX - fsTouchPanPrevX) / fsZoom;
        const dy = (e.touches[0].clientY - fsTouchPanPrevY) / fsZoom;
        fsPanX += dx;
        fsPanY += dy;
        commitTransform();
        fsTouchPanPrevX = e.touches[0].clientX;
    ...
    fsOverlay.addEventListener('touchend', e => {
      if (e.touches.length < 2) { fsLastPinchDist = null; fsLastPinchMidX = null; fsLastPinchMidY = null; }
      if (e.touches.length === 0) fsTouchPanning = false;
    }, { passive: true });
