...
(function () {
  ...
  (function initZoomPan() {
    ...
    // Use transform-origin top-left + translate so the frame
    // scales from its own top-left corner — this keeps pixel
    // boundaries aligned and avoids sub-pixel blurring.
    a4Frame.style.transformOrigin = '0 0';

    function clampPan(zoom) {
      const frameW = a4Frame.offsetWidth;
      const frameH = a4Frame.offsetHeight;
      // The scaled frame is frameW*zoom wide and frameH*zoom tall.
      // We can only translate within the difference between scaled and original size.
      // panX/panY are the CSS translate values (in CSS px, pre-scale).
      const maxPanX = frameW * (zoom - 1);
      const maxPanY = frameH * (zoom - 1);
      panX = Math.max(-maxPanX, Math.min(0, panX));
      panY = Math.max(-maxPanY, Math.min(0, panY));
    }

    function applyTransform() {
      // Round to whole pixels to avoid blurry sub-pixel rendering
      const tx = Math.round(panX);
      const ty = Math.round(panY);
      a4Frame.style.transform = `scale(${zoomLevel}) translate(${tx / zoomLevel}px, ${ty / zoomLevel}px)`;
