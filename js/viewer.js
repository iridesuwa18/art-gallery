...
(function () {
  ...
  function equaliseMultiHeights(pageEl, size) {
    ...
    const a4H = pageEl.offsetHeight || a4Frame.offsetHeight || 1;
  ...
  function buildMultiCell(page, pageIdx) {
    ...
    pageEl.classList.add('art-page--in-multi');
    frame.appendChild(pageEl);

    // Mirror --a4-w/--a4-h directly onto the art-page element so its
    // calc() padding values resolve correctly even before the frame's
    // vars propagate via inheritance.
    function syncPageVars() {
      const aw = frame.style.getPropertyValue('--a4-w');
      const ah = frame.style.getPropertyValue('--a4-h');
      if (aw) pageEl.style.setProperty('--a4-w', aw);
      if (ah) pageEl.style.setProperty('--a4-h', ah);
    }
