  ...
  function buildFullscreenOverlay() {
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
        ...
        if (ni < pages.length) { currentIdx = fsIsMulti ? Math.floor(ni / PAGES_PER_SPREAD) * PAGES_PER_SPREAD : ni; renderFullscreen(); if (multiMode) renderMultiSpread(); else renderPage(pages[currentIdx]); updateNav(pages.length); }
      }
      if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
        const ni = currentIdx - step;
        if (ni >= 0) { currentIdx = Math.max(0, fsIsMulti ? Math.floor(ni / PAGES_PER_SPREAD) * PAGES_PER_SPREAD : ni); renderFullscreen(); if (multiMode) renderMultiSpread(); else renderPage(pages[currentIdx]); updateNav(pages.length); }
      }
