...
function buildBrushShapeSection() {
  ...
  // ── Helper: build one inspector panel ──────────────────────────
  function buildInspectorPanel({ isCompare = false } = {}) {
    const panel = document.createElement('div');
    panel.className = 'brush-inspector-panel' + (isCompare ? ' brush-inspector-compare' : '');

    const hdr = document.createElement('div');
    hdr.className = 'brush-chooser-header';

    const title = document.createElement('span');
    title.className = 'brush-chooser-title';
    title.textContent = isCompare ? 'Compare' : 'Brush Inspector';

    const actions = document.createElement('div');
    actions.className = 'brush-chooser-actions';
    ...
    copyBtn.className = 'panel-btn sm chooser-copy-btn';
    copyBtn.title = 'Copy brush shape name';
    copyBtn.style.display = 'none';
    const copyIconSVG = `<svg width="13" height="13" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    dlBtn.className = 'panel-btn sm';
    dlBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M8 2v9M5 8l3 3 3-3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/><path d="M2 13h12" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg> PNG';
    dlBtn.title = 'Download this shape as PNG';
    dlBtn.style.display = 'none';

    if (isCompare) {
      // Close button instead of compare-add
      const closeBtn = document.createElement('button');
      closeBtn.className = 'chooser-compare-close';
      closeBtn.title = 'Close comparison';
      closeBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M2 2l12 12M14 2L2 14" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>';
      title.appendChild(closeBtn);
      closeBtn.addEventListener('click', () => {
        panel.classList.remove('visible');
        setTimeout(() => panel.remove(), 260);
        chooserWrap.classList.remove('has-compare');
      });
    }

    actions.appendChild(copyBtn);
    actions.appendChild(medSel);
    ...
    const canvas = document.createElement('canvas');
    canvas.className = 'brush-chooser-canvas';
    canvas.width = canvas.height = 400;
    canvas.style.display = 'none';
    previewWrap.appendChild(canvas);

    const meta = document.createElement('div');
    meta.className = 'brush-chooser-meta';
    meta.style.display = 'none';
    const shapeName = document.createElement('div');
    shapeName.className = 'brush-chooser-shape-name';
    const shapeDesc = document.createElement('div');
    shapeDesc.className = 'brush-chooser-shape-desc';
    meta.appendChild(shapeName);
    meta.appendChild(shapeDesc);
    ...
    medSel.addEventListener('change', () => {
      ...
      drawBrushShape(canvas, shape, { label: false, unified: true });
      shapeName.textContent = shape.label;
      shapeDesc.textContent = med.brushShape + (shape.desc ? ' — ' + shape.desc : '');

      dlBtn.onclick = () => {
        const c = document.createElement('canvas');
        c.width = c.height = 1080;
        drawBrushShape(c, shape, { silhouette: true });
        c.toBlob(blob => {
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url;
      ...
      infoBtn.onclick = () => openMediumDetail(medName);
    });

    copyBtn.addEventListener('click', () => {
      const text = currentMed ? `${currentMed.name} — ${currentMed.brushShape}` : '';
      if (!text) return;
      navigator.clipboard.writeText(text).then(() => {
        copyBtn.innerHTML = `<svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M2.5 8.5L6 12L13.5 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    ...
    panel._redraw = () => {
      if (canvas.style.display !== 'none' && currentShape) {
        drawBrushShape(canvas, currentShape, { label: false, unified: true });
      }
    };
  ...
  // ── Primary panel ──
  const primaryPanel = buildInspectorPanel({ isCompare: false });

  // ── Compare button (sits in primary panel header, left of title) ──
  const primaryHdr = primaryPanel.querySelector('.brush-chooser-header');
  const primaryTitle = primaryHdr.querySelector('.brush-chooser-title');

  const compareAddBtn = document.createElement('button');
  compareAddBtn.className = 'chooser-compare-add';
  compareAddBtn.title = 'Compare with another medium';
  compareAddBtn.innerHTML = '<svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M5 1v8M1 5h8" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>';

  primaryTitle.insertBefore(compareAddBtn, primaryTitle.firstChild);

  // ── Panels container ──
  const panelsRow = document.createElement('div');
  panelsRow.className = 'brush-inspector-row';
  panelsRow.appendChild(primaryPanel);

  compareAddBtn.addEventListener('click', () => {
    if (chooserWrap.querySelector('.brush-inspector-compare')) return; // only one at a time
    const cmp = buildInspectorPanel({ isCompare: true });
    panelsRow.appendChild(cmp);
    chooserWrap.classList.add('has-compare');
    // Trigger slide-in animation next frame
    requestAnimationFrame(() => cmp.classList.add('visible'));
  });

  chooserWrap.appendChild(panelsRow);
  ...
  if (themeBtn) {
    themeBtn.addEventListener('click', () => {
      setTimeout(() => {
        ...
        panelsRow.querySelectorAll('.brush-inspector-panel').forEach(p => p._redraw?.());
      }, 30);
    });
  }

  // Keep legacy refs alive (used nowhere else but keep for safety)
  const currentChooserMedium = null;
  const currentChooserShape = null;
