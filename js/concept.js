...
function drawBrushShape(canvas, shape, opts = {}) {
  ...
  const isUnified    = opts.unified === true; // same theme colour, no separate handle/ferrule tones
  ...
  // Store palette on ctx for helpers
  // silhouette → all white; unified → match brush; normal → distinct handle/ferrule tones
  ctx._ferruleCol = (isSilhouette || isUnified) ? brushColour : (isDark ? '#5a5650' : '#8a8680');
  ctx._handleCol  = (isSilhouette || isUnified) ? brushColour : (isDark ? '#8a6e3a' : '#c8a96a');
  ctx._handleW    = Math.round(W * 0.013);
  ctx._noHandleParts = true; // ferrule and handle suppressed — bristle shape only
