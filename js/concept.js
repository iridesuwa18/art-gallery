...
function drawBrushShape(canvas, shape, opts = {}) {
  ...
  // Brush colour
  const brushColour = isDark ? '#edeae3' : '#1a1814';
  ctx.fillStyle = brushColour;
  ctx.strokeStyle = brushColour;

  // Store palette on ctx for helpers
  ctx._ferruleCol = isDark ? '#5a5650' : '#8a8680';
  ctx._handleCol  = isDark ? '#8a6e3a' : '#c8a96a';
...
function buildShapeThumbnail(shape) {
  ...
  drawBrushShape(canvas, shape, { label: false });
  return canvas;
}
...
function buildBrushShapeSection() {
  ...
  medSel.addEventListener('change', () => {
    ...
    drawBrushShape(chooserCanvas, shape, { label: false });
    ...
    chooserDlBtn.onclick = () => {
      ...
      drawBrushShape(canvas1080, shape);
    ...
    themeBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        ...
        if (chooserCanvas.style.display !== 'none' && currentChooserShape) {
          drawBrushShape(chooserCanvas, currentChooserShape, { label: false });
        }
      }, 30);
    });
  }
