...
function buildPageFolder(page, index) {
  ...
  // Prevent layout-select and delete button click from toggling
  header.addEventListener('click', e => {
    if (e.target.closest('.layout-select') || e.target.closest('.delete-page-btn')) return;
    folder.classList.toggle('open');
  });

  // Layout select (stop propagation so it doesn't toggle folder)
