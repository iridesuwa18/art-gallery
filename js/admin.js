...
function renderPagesList() {
  ...
  sections.forEach((sec, secIdx) => {
    const letter = sectionLetter(secIdx);
    ...
    secHeader.innerHTML = `
      <span class="pages-section-bar-letter">${esc(letter)}</span>
      <span class="pages-section-bar-name">${esc(sec.name || `Section ${secIdx + 1}`)}</span>
    `;
    pagesList.appendChild(secHeader);

    let pageSubIdx = 0; // running counter for labelling within this section
    ...
    if (subsections.length > 0) {
      ...
      // Pages in section but not in any subsection
      ...
      // No subsections — flat list under section
