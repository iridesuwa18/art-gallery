...
function buildPageFolder(page, index, label) {
  ...
  // Inject move-up / move-down buttons into pf-right (before existing buttons)
  const pfRight = folder.querySelector('.pf-right');
  if (pfRight) {
    const moveUpBtn = document.createElement('button');
    moveUpBtn.className = 'icon-btn move-page-up-btn';
    moveUpBtn.title = 'Move page up';
    moveUpBtn.innerHTML = `<svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2 8l4-4 4 4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    const moveDnBtn = document.createElement('button');
    moveDnBtn.className = 'icon-btn move-page-dn-btn';
    moveDnBtn.title = 'Move page down';
    moveDnBtn.innerHTML = `<svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2 4l4 4 4-4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    pfRight.insertBefore(moveUpBtn, pfRight.firstChild);
    pfRight.insertBefore(moveDnBtn, pfRight.children[1] || null);
  }
  ...
  header.addEventListener('click', e => {
    if (e.target.closest('.pf-right') || e.target.closest('.move-page-up-btn') || e.target.closest('.move-page-dn-btn')) return;
    folder.classList.toggle('open');
  });
  ...
  function rebuildPageSettings() {
    ...
    if (layout === 'flood') {
      // ── Helper: push flood CSS vars to live DOM immediately ──
      function applyFloodVarsLive() {
        const floodWrap = document.querySelector('.artwork-flood');
        if (!floodWrap) return;
        const caption = floodWrap.querySelector('.flood-caption');
        if (caption) {
          caption.style.setProperty('--flood-h-ratio', page.floodHeightRatio !== undefined ? page.floodHeightRatio : 0.18);
          caption.style.setProperty('--flood-text-scale', page.floodTextScale !== undefined ? page.floodTextScale : 1.0);
        }
        floodWrap.style.setProperty('--flood-bg', page.floodBoxBg || 'rgba(255,255,255,0.92)');
        floodWrap.style.setProperty('--flood-fg', page.floodBoxFg || '#1a1814');
      }
      ...
      const heightRow = makeSettingRow('Text Box Height', [
        makeSlider('flood-height', 5, 50, Math.round((page.floodHeightRatio !== undefined ? page.floodHeightRatio : 0.18) * 100),
          v => { page.floodHeightRatio = v / 100; applyFloodVarsLive(); scheduleAutosave(); }, '%')
      ]);
      ...
      const scaleRow = makeSettingRow('Text Size Scale', [
        makeSlider('flood-scale', 50, 200, Math.round((page.floodTextScale !== undefined ? page.floodTextScale : 1.0) * 100),
          v => { page.floodTextScale = v / 100; applyFloodVarsLive(); scheduleAutosave(); }, '%')
      ]);

      // ── Flood: box background colour ──
      const boxBgRow = makeFloodColourRow('Box Background', page.floodBoxBg || 'rgba(255,255,255,0.92)', [
        { label: 'White',        value: 'rgba(255,255,255,0.92)' },
        { label: 'Off-white',    value: 'rgba(250,248,245,0.92)' },
        { label: 'Black',        value: 'rgba(15,13,11,0.88)'   },
        { label: 'Charcoal',     value: 'rgba(42,40,38,0.88)'   },
        { label: 'Transparent',  value: 'rgba(0,0,0,0)'          },
      ], v => { page.floodBoxBg = v; applyFloodVarsLive(); scheduleAutosave(); });

      // ── Flood: text colour ──
      const boxFgRow = makeFloodColourRow('Text Colour', page.floodBoxFg || '#1a1814', [
        { label: 'Dark',   value: '#1a1814' },
        { label: 'White',  value: '#f0ede6' },
        { label: 'Black',  value: '#000000' },
        { label: 'Grey',   value: '#888888' },
      ], v => { page.floodBoxFg = v; applyFloodVarsLive(); scheduleAutosave(); });

      pageSettingsWrap.appendChild(cornerRow);
      pageSettingsWrap.appendChild(heightRow);
      pageSettingsWrap.appendChild(scaleRow);
      pageSettingsWrap.appendChild(boxBgRow);
      pageSettingsWrap.appendChild(boxFgRow);
  ...
  const moveUpBtn = folder.querySelector('.move-page-up-btn');
  const moveDnBtn = folder.querySelector('.move-page-dn-btn');
  if (moveUpBtn) {
    moveUpBtn.disabled = index === 0;
    moveUpBtn.addEventListener('click', e => {
      e.stopPropagation();
      if (index === 0) return;
      const pages = localConfig.pages;
  ...
  if (moveDnBtn) {
    moveDnBtn.disabled = index === (localConfig.pages.length - 1);
    moveDnBtn.addEventListener('click', e => {
      e.stopPropagation();
      const pages = localConfig.pages;
  ...
  // "Add page after this" button at the bottom of folder body
  const addPageHereBtn = document.createElement('button');
  addPageHereBtn.className = 'panel-btn sm add-page-here-btn';
  addPageHereBtn.type = 'button';
  addPageHereBtn.innerHTML = `<svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg> Add page after this`;
  addPageHereBtn.addEventListener('click', () => {
    const newPage = {
      id: 'page_' + Date.now(),
      layout: 'large',
      artworks: [],
      sectionId: page.sectionId || null,
      subsectionId: page.subsectionId || null,
    };
    // Insert after current page in localConfig.pages
    const realIdx = localConfig.pages.indexOf(page);
    if (realIdx === -1) {
      localConfig.pages.push(newPage);
    } else {
      localConfig.pages.splice(realIdx + 1, 0, newPage);
    }
    renderPagesList();
    scheduleAutosave();
    // Auto-open the new page's folder for editing
    setTimeout(() => {
      const newFolder = pagesList.querySelector(`[data-page-id="${newPage.id}"]`);
      if (newFolder) {
        newFolder.classList.add('open');
        newFolder.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    }, 60);
  });
  folderBody.appendChild(addPageHereBtn);
...
// ── Flood caption colour picker (box bg or text colour) ──────
function makeFloodColourRow(label, currentVal, presets, onChange) {
  const wrap = document.createElement('div');
  ...
  // Helper: normalise colour strings for comparison
  function normalise(v) { return (v || '').replace(/\s/g, '').toLowerCase(); }

  presets.forEach(preset => {
    const swatch = document.createElement('button');
    swatch.className = 'page-bg-swatch' + (normalise(preset.value) === normalise(currentVal) ? ' active' : '');
    swatch.title = preset.label;
    swatch.type = 'button';
    if (preset.value && preset.value !== 'rgba(0,0,0,0)') {
  ...
  // Custom colour input
  // Convert rgba to hex approximation for the color input
  function rgbaToHex(v) {
    const m = (v || '').match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
    if (!m) return v && v.startsWith('#') ? v : '#ffffff';
  ...
  customInput.value = rgbaToHex(currentVal);
...
function buildFvItem(file) {
  ...
  renameInput.className = 'fv-rename-input panel-input';
  renameInput.style.display = 'none';
  renameInput.style.fontSize = '11px';
  renameInput.style.width = '100%';
  renameInput.style.marginTop = '4px';
  ...
  renameBtn.addEventListener('click', e => {
    ...
    confirmRenameBtn.style.display = '';
    cancelRenameBtn.style.display = '';
    deleteBtn.style.display = 'none';
  });

  // ── Confirm rename ─────────────────────────────────────────
  const confirmRenameBtn = document.createElement('button');
  confirmRenameBtn.className = 'fv-action-btn';
  confirmRenameBtn.textContent = 'Save';
  confirmRenameBtn.style.display = 'none';
  confirmRenameBtn.addEventListener('click', async e => {
    e.stopPropagation();
    const extMatch = file.name.match(/(\.[^.]+)$/);
    const ext = extMatch ? extMatch[1].toLowerCase() : '.jpg';
    const newBase = renameInput.value.trim()
      .toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
    if (!newBase) { alert('Please enter a valid filename.'); return; }
    const newName = newBase + ext;
    if (newName === file.name) { cancelRename(); return; }
    confirmRenameBtn.textContent = '…';
    confirmRenameBtn.disabled = true;
    try {
      await renameRepoImage(file.name, file.sha, newName);
      loadFileViewer();
    } catch(err) {
  ...
  cancelRenameBtn.className = 'fv-action-btn';
  ...
  cancelRenameBtn.addEventListener('click', e => { e.stopPropagation(); cancelRename(); });
  ...
  actions.appendChild(confirmRenameBtn);
  actions.appendChild(cancelRenameBtn);
...
async function renameRepoImage(oldName, oldSha, newName) {
  const { owner, repo, branch, token } = ghCredentials;
  const ghHeaders = buildGhHeaders(token);
  ...
  const createRes = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/contents/images/${encodeURIComponent(newName)}`,
    {
      method: 'PUT',
      headers: jsonHeaders,
      body: JSON.stringify({ message: `[artbook] Rename ${oldName} → ${newName}`, content, branch })
    }
  );
  if (!createRes.ok) {
    const err = await createRes.json().catch(() => ({}));
    throw new Error(err.message || 'Could not create renamed file');
  }
  ...
  // 4. Update any artwork URLs in localConfig that reference the old filename
  const oldParam = encodeURIComponent(oldName);
  const newParam = encodeURIComponent(newName);
  (localConfig.pages || []).forEach(page => {
    (page.artworks || []).forEach(art => {
      if (art.url && art.url.includes(`f=${oldParam}`)) {
        art.url = art.url.replace(`f=${oldParam}`, `f=${newParam}`);
      }
    });
  });
