...
// Desktop: native double-click on invisible overlay
adminTrigger.addEventListener('dblclick', () => openAdmin());

// Mobile: two taps within 400ms
let lastTap = 0;
...
function buildPageFolder(page, index) {
  ...
  // Fold/unfold — clicking the left side or chevron toggles, not the right controls
  const header = folder.querySelector('.page-folder-header');
  const pfLeft = folder.querySelector('.pf-left');
  pfLeft.addEventListener('click', () => {
    folder.classList.toggle('open');
  });
  // Also allow clicking the header background (but not right-side controls)
...
function buildArtworkRow(art, page, artIndex) {
  ...
  info.innerHTML = `
    ...
    <div class="artwork-row-sub">${esc(art.date || '')}${art.date ? ' · ' : ''}${art.textPosition === 'left' ? 'Text L' : 'Text R'}</div>
  `;
